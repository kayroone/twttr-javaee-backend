...
public final class ObjectBuilder<T> {
    ...
    static <V> ObjectBuilder<V> fromGenericType(final Class<?> subclass, final Class<?> superclass) {
        return fromGenericType(subclass, superclass, 0);
    }

    static <V> ObjectBuilder<V> fromGenericType(final Class<?> subclass, final Class<?> superclass, final int
            parameterIndex) {
